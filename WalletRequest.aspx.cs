...
public partial class WalletRequest : System.Web.UI.Page
{
    ...
    private void FillPaymode()
    {
        try
        {
            strQuery = "SELECT * FROM " + Obj.dBName + "..M_PayModeMaster WHERE ActiveStatus='Y'   order by Pid";
    ...
    protected bool CheckDDno()
    {
        ...
        if (Convert.ToInt32(DdlPaymode.SelectedValue) != 8)
        {
            ...
            {
                s = "select * from " + Obj.dBName + "..WalletReqINR where ChqNo = '" + (TxtDDNo.Text) + "' and IsApprove <> 'R'";
    ...
    protected void SaveRequest()
    {
        try
        {
            ...
            int updateeffect = SqlHelper.ExecuteNonQuery(constr, CommandType.Text, StrSql);
            if (updateeffect > 0)
            {
                string flnm1 = "";
     ...
     "'Amount: " + TxtAmount.Text + "',Getdate()," + Session["FormNo"] + ")";
                        ...
                        int ReqNo;
                        str = " Select Max(ReqNo) as ReqNo FROM " + Obj.dBName + "..WalletReqINR WHERE Formno='" + Session["Formno"] + "' AND Amount='" + TxtAmount.Text + "'";
                        dt1 = new DataTable();
            ...
            else
            {
                ...
                string urlStr = "WalletRequest.aspx";
